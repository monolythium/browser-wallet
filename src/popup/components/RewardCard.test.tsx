...
describe("RewardCard — auto-compound toggle", () => {
  it("renders the On/Off toggle from rewards.autoCompound when the parent wires it (live read)", () => {
    ...
    expect(off).toContain("Auto-compound");
    ...
    const on = renderToStaticMarkup(
      <RewardCard
        {...baseProps}
        rewards={rewardsView({ autoCompound: true, totalAmountWei: "0x64" })}
        onSetAutoCompound={() => {}}
      />,
    );
  ...
  it("hides the toggle when the parent doesn't wire it (opt-in; legacy unchanged)", () => {
    const html = renderToStaticMarkup(
      <RewardCard {...baseProps} rewards={rewardsView({ autoCompound: true })} />,
    );
    expect(html).not.toContain("Auto-compound");
    expect(html).not.toContain('role="switch"');
  });

  it("hides the toggle for a mock/illustrative read (no-mock — never flip a fabricated flag)", () => {
    const html = renderToStaticMarkup(
      <RewardCard
        {...baseProps}
        isMock={true}
  ...
  it("shows an Updating… state while a toggle is in flight", () => {
    const html = renderToStaticMarkup(
      <RewardCard
        {...baseProps}
        rewards={rewardsView({ autoCompound: false })}
        onSetAutoCompound={() => {}}
        autoCompoundPending={true}
      />,
    );
